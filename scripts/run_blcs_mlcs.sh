...
if [[ -z $QLANG || -z $DLANG || -z $PROB || -z $SEED ]]; then
  echo 'one or more variables are undefined'
  exit 1
fi

PROJECT_HOME=/path/to/projects/CodeSwitchCLIR
DATA_HOME=/path/to/data_home/CLWE-CS/translation_prob=$PROB/$QLANG$DLANG
OUT=/path/to/model_home/CLWE-CS/seed\_$SEED/$PROB/$QLANG$DLANG
